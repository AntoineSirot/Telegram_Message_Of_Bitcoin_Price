...
declare -a VALUES # Array that will stack prices of Bitcoin and reset everyday
declare -a PRICES # Array that will stack every price of Bitcoin since the launching
...
VALUES[$(($COMPT))]=$PRICE # Keep every value that will be necessary for total, mean, max and min (reset everyday)
PRICES[$(($COMPT + $NUMBER_DAYS * $DAY ))]=$PRICE # Keep every values 
...
if (( $(($COMPT % $HOUR)) == 0 )); then # 60 min = 1 hour
        time=$(date) # Adding the date for the message
        curl --data  chat_id=$your_chat_id --data-urlencode "text= $time
Bitcoin's price now  : \$${PRICE} !" "https://api.telegram.org/bot$your_telegram_token/sendMessage?parse_mode=HTML" # Sending a Telegram message with current Bitcoin Price, its Max, Min and Mean of the last 24 hours 
...
Higher price : \$${max}
Maximum gap : \$${MAX_GAP}
Evolution in the last 24h : ${DAILY_GAP}% " "https://api.telegram.org/bot$your_telegram_token/sendMessage?parse_mode=HTML" # Sending a Telegram message with current Bitcoin Price, its Max, Min and Mean of the last 24 hours 
        echo "" # Printing nothing so next print will be on a new line
